...
func (usecase *forumUsecase) ReplyForum(requestURI string, requestMethod string, forumID string, requestBody io.Reader, urlQuery string, requestHeader map[string][]string) (*http.Response, error) {
	...
	resp, err := usecase.forumRepository.ReplyForum(forumID, requestBody, urlQuery, requestHeader)
